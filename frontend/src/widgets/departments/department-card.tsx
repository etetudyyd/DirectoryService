...
export default function DepartmentCard({ department, onEdit }: Props) {
  ...
  const formattedCreatedAt = formatDateTime(department.createdAt);
  const formattedUpdatedAt = formatDateTime(department.updatedAt);
  const formattedDeletedAt = formatDateTime(department.deletedAt);

  return (
    <Link href={`${routes.departments}/${department.id}`}>
